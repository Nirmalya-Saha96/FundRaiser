...
class NewNpos extends Component {
  state = {
    minimumContribution: "",
    name: "",
    description: "",
 ...
 onSubmit = async (e) => {
     ...
     try {
       ...
       await factory.methods
         .createNPOCampaign(this.state.minimumContribution, this.state.name, this.state.description, this.state.hash)
         .send({
           from: accounts[0]
         });

      Router.pushRoute('/npos');
     } catch (err) {
  ...
  render() {
    return (
      <Layout>
        <h2>Create a Scholarship FundRaiser</h2>
